...
def SetChoice(userid,roomid,choice,choicename):
	try:
		...
		with conn.cursor() as cursor:
			...
			if (balance > 5) or (balance <= 10):
				balance = 5

			cursor.execute(F"UPDATE goldenusers SET last_choice = '{choice}' WHERE userid = '{userid}'")
			message = F"{username}, введи ставку на {choicename}:"
			methods.messages.send(peer_id=roomid,random_id=0,message=message,keyboard=menu.main())
	except Exception:
		traceback.print_exc()
			...
			if seconds >= 85:
				...
				if textchoice != 'None':
					...
					if (maxbet <= 1000) and (bet > 99):
						...
						cursor.execute(F"UPDATE goldenusers SET last_choice = 'None',last_number = 'None' WHERE userid = '{userid}'")
						message = F"{name}, максимальная сумма ставок: 1 000 коинов, минимальная: 10."
						messages_send(roomid,message,menu.main())
	except Exception:
		traceback.print_exc()
...
def GetBank(roomid):
	try:
		...
		with conn.cursor() as cursor:
			...
			seconds = (timenow - roomrolltime).seconds
			timez = str(timedelta(seconds=90 - seconds))[3::]
			if play == 'No':
